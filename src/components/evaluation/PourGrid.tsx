		...
		<table className="w-full text-sm" style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
			...
			<tbody>
				{principleScores.map((ps, i) => {
					...
					const bottomBorder = isLast ? '' : 'border-b border-gray-200'

					return (
						<tr key={ps.principle}>
							<td className={`border-r border-gray-200 px-5 py-3 font-medium text-gray-900 ${bottomBorder}`}>
								{displayName}
							</td>
							<td className={`border-r border-gray-200 px-5 py-3 text-right font-bold tabular-nums text-gray-900 ${bottomBorder}`}>
								{ps.score}%
							</td>
							<td className={`border-r border-gray-200 px-5 py-3 text-right tabular-nums text-gray-700 ${bottomBorder}`}>
								{ps.issueCount === 0 ? '\u2014' : ps.issueCount}
							</td>
							<td className={`px-5 py-3 text-right tabular-nums text-gray-700 ${bottomBorder}`}>
								{ps.needsReviewCount === 0 ? '\u2014' : ps.needsReviewCount}
							</td>
						</tr>
					)
				})}
