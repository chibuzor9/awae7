...
export async function GET(
  ...
  try {
    ...
    if (evaluation.rawResults) {
      ...
      const violations: ViolationItem[] = evaluation.violations.map((v: any) => ({
        id: v.id,
        ruleId: v.ruleId,
        ...
        wcagPrinciple: v.wcagPrinciple as
          ...
          | "Understandable"
          | "Robust",
        severity: v.severity as "critical" | "serious" | "moderate" | "minor",
    ...
    return NextResponse.json(
      {
        error:
          "Failed to fetch evaluation details. Please try again later.",
      },
      { status: 500 }
    );
  }
