...
export function generateEndUserReport(
  result: EvaluationResult
): EndUserReport {
...
const REMEDIATION_MAP: Record<string, string> = {
  ...
  label:
    "Add a <label> element associated with each form input using the 'for' attribute matching the input's 'id', or wrap the input inside a <label>.",
  "link-name":
  ...
  region:
    "Ensure all page content is contained within landmark regions (e.g., <main>, <nav>, <header>, <footer>, or ARIA landmarks).",
  bypass:
    'Provide a "skip to main content" link as the first focusable element on the page so keyboard users can bypass repetitive navigation.',
  "aria-allowed-attr":
...
const PLAIN_LANGUAGE_MAP: Record<string, string> = {
  ...
  label:
    "Some form fields are missing labels, which makes it hard to know what information to enter.",
  "link-name":
  ...
  region:
    "Some content on this page is not inside clearly defined sections, making navigation harder.",
  bypass:
    "This page is missing a 'skip to content' link, making keyboard users tab through the entire navigation every time.",
  "aria-allowed-attr":
